...
export class NewsFeedController {
  ...
  async createNewsFeed(
    ...
    const entity = await this.newsFeedService.createNewsFeed({
      ...newsFeedDto,
      user_id: user.id,
    });
  ...
  async getNewsFeeds(
    ...
    const entity = await this.newsFeedService.getAllNewsFeed(
      user.id,
      pageOptionsDto,
    );
  ...
  @Get('/user/:id')
  @Auth([RoleType.USER])
  async getNewsFeedByUser(@Param('id') id: string) {
    const entity = await this.newsFeedService.getOnePostPerFollower(id);
